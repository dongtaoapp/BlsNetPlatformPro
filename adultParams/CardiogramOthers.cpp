...
namespace jysoft { namespace simulator { namespace ecg {
	...
	CCardiogramOthers::CCardiogramOthers(IStorage *pRootStorage )
	{
		USES_CONVERSION;
		if(pRootStorage != NULL)
		{
			pRootStorage->OpenStorage(L"Other Rhythm",NULL,STGM_READ|STGM_SHARE_EXCLUSIVE,0,0, &m_pI12LeadsStorage);
		}
		if( m_pI12LeadsStorage != NULL )
		{
			...
			for (int nStep = 0; nStep < 9; ++nStep)
			{
				...
				m_pEcgLeadParam[nStep] = new CCardiogramOthersParam_I12( pEcgLeadsStorage[nStep] );
			}
		}
	}

	CCardiogramOthers::~CCardiogramOthers(void)
	{
	}

    bool CCardiogramOthers::IsCanParamExpress(BasicRhythm eRhythm, int uHR)
	{
		bool bRtn = false;
		switch(eRhythm)
		{
	...
	// 参数：  
	// 返回值: 
    bool CCardiogramOthers::IsCanExpressCardiogramParam(BasicRhythm eRhythm, int uHR)
	{
		return CCardiogramOthers::IsCanParamExpress( eRhythm, uHR );
	}
	...
	//         
	// 返回值:
    bool CCardiogramOthers::LoadCardiogramLeadsParam( BasicRhythm eRhythm, int uHR, short sConduct, short sExtendParam )
	{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramOthers::ReLoadCardiogramParam()
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramOthers::SeteRefurbishDataRange(_eRefurbishDataRange eRange)
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	_eRefurbishDataRange CCardiogramOthers::GeteRefurbishDataRange()
	{
