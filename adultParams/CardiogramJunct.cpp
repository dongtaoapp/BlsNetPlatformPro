...
namespace jysoft { namespace simulator { namespace ecg {
	...
	CCardiogramJunct::CCardiogramJunct(IStorage *pRootStorage )
	{
		USES_CONVERSION;
		if(pRootStorage != NULL)
		{
			pRootStorage->OpenStorage(L"Junct",NULL,STGM_READ|STGM_SHARE_EXCLUSIVE,0,0, &m_pI12LeadsStorage);
		}
		if( m_pI12LeadsStorage != NULL )
		{
			...
			for (int nStep = 0; nStep < 9; ++nStep)
			{
				...
				m_pEcgLeadParam[nStep] = new CCardiogramJunctParam_I12( pEcgLeadsStorage[nStep] );
			}
		}
	}

	CCardiogramJunct::~CCardiogramJunct(void)
	{
	}

    bool CCardiogramJunct::IsCanParamExpress(BasicRhythm eRhythm, int uHR)
	{
		bool bRtn = false;
		switch(eRhythm)
		{
	...
	// 参数：  
	// 返回值: 
    bool CCardiogramJunct::IsCanExpressCardiogramParam(BasicRhythm eRhythm, int uHR)
	{
		return CCardiogramJunct::IsCanParamExpress( eRhythm, uHR );
	}
	...
	//         
	// 返回值:
    bool CCardiogramJunct::LoadCardiogramLeadsParam( BasicRhythm eRhythm, int uHR, short sConduct, short sExtendParam )
	{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramJunct::ReLoadCardiogramParam()
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramJunct::SeteRefurbishDataRange(_eRefurbishDataRange eRange)
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	_eRefurbishDataRange CCardiogramJunct::GeteRefurbishDataRange()
	{
