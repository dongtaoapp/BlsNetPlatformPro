...
namespace jysoft { namespace simulator { namespace ecg {
	...
	CCardiogramIdiov::CCardiogramIdiov(IStorage *pRootStorage )
	{
		USES_CONVERSION;
		if(pRootStorage != NULL)
		{
			pRootStorage->OpenStorage(L"Idiov",NULL,STGM_READ|STGM_SHARE_EXCLUSIVE,0,0, &m_pI12LeadsStorage);
		}
		if( m_pI12LeadsStorage != NULL )
		{
			...
			for (int nStep = 0; nStep < 9; ++nStep)
			{
				...
				m_pEcgLeadParam[nStep] = new CCardiogramIdiovParam_I12( pEcgLeadsStorage[nStep] );
			}
		}
	}

	CCardiogramIdiov::~CCardiogramIdiov(void)
	{
	}

    bool CCardiogramIdiov::IsCanParamExpress(BasicRhythm eRhythm, int uHR)
	{
		bool bRtn = false;
		switch(eRhythm)
		{
	...
	// 参数：  
	// 返回值: 
    bool CCardiogramIdiov::IsCanExpressCardiogramParam(BasicRhythm eRhythm, int uHR)
	{
		return CCardiogramIdiov::IsCanParamExpress( eRhythm, uHR );
	}
	...
	//         
	// 返回值:
    bool CCardiogramIdiov::LoadCardiogramLeadsParam( BasicRhythm eRhythm, int uHR, short sConduct, short sExtendParam )
	{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramIdiov::ReLoadCardiogramParam()
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramIdiov::SeteRefurbishDataRange(_eRefurbishDataRange eRange)
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	_eRefurbishDataRange CCardiogramIdiov::GeteRefurbishDataRange()
	{
