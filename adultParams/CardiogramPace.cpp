...
namespace jysoft { namespace simulator { namespace ecg {
	...
	CCardiogramPace::CCardiogramPace( IStorage *pRootStorage )
	{
		USES_CONVERSION;
		if(pRootStorage != NULL)
		{
			pRootStorage->OpenStorage(L"Pacing",NULL,STGM_READ|STGM_SHARE_EXCLUSIVE,0,0, &m_pI12LeadsStorage);
		}
		if( m_pI12LeadsStorage != NULL )
		{
			...
			for (int nStep = 0; nStep < 9; ++nStep)
			{
				...
				m_pEcgLeadParam[nStep] = new CCardiogramPaceParam_I12( pEcgLeadsStorage[nStep] );
			}
		}
	}

	CCardiogramPace::~CCardiogramPace(void)
	{
	}

    bool CCardiogramPace::IsCanParamExpress(BasicRhythm eRhythm, int uHR)
	{
		bool bRtn = false;
		switch(eRhythm)
		{
	...
	// 参数：  
	// 返回值: 
    bool CCardiogramPace::IsCanExpressCardiogramParam(BasicRhythm eRhythm, int uHR)
	{
		return CCardiogramPace::IsCanParamExpress( eRhythm, uHR );
	}
	...
	//         
	// 返回值:
    bool CCardiogramPace::LoadCardiogramLeadsParam( BasicRhythm eRhythm, int uHR, short sConduct, short sExtendParam )
	{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramPace::ReLoadCardiogramParam()
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	// 参数：  
	// 返回值: 
	void CCardiogramPace::SeteRefurbishDataRange(_eRefurbishDataRange eRange)
	{
		for (int index = 0; index < _LEAD_SUM /*9*/; ++index)
		{
	...
	_eRefurbishDataRange CCardiogramPace::GeteRefurbishDataRange()
	{
